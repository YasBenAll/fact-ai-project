...
import pickle
import functools
import baselines.fairlearn.moments as moments
...
class _Lagrangian:
    ...
    def solve_linprog(self, nu, starting_tol=1e-12, max_tol=1e-9):
        n_hs = len(self.hs)
        n_cons = len(self.cons.index)
        if hasattr(self, 'last_linprog_res') and self.last_linprog_n_hs == n_hs:
        ...
        while tol <= max_tol:
            try:
                res = opt.linprog(c, A_ub=A_ub, b_ub=b_ub, A_eq=A_eq, b_eq=b_eq, options={'tol':tol})
                break
            except ValueError as e:
                tol = tol * 10
                exc = e
        if tol > max_tol and not(exc is None):
            raise(exc)
        ...
        while tol <= max_tol:
            try:
                res_dual = opt.linprog(dual_c, A_ub=dual_A_ub, b_ub=dual_b_ub, bounds=dual_bounds, options={'tol':tol})
                break
            except ValueError as e:
                tol = tol * 10
                exc = e
    ...
    def best_h(self, lambda_vec):
        ...
        # print(np.unique(redY), np.unique(signed_weights))
...
def expgrad(dataX, dataA, dataY, learner, cons=moments.DP(), eps=0.01,
    ...
    B = 1/eps
    # print(np.unique(dataA), np.unique(dataY))
    ...
    for t in range(0, T):
        ...
        if (t == 0) or not _RUN_LP_STEP:
            ...
            try:
                Q_LP, lambda_LP, res_LP = lagr.solve_linprog(nu, starting_tol=1e-12, max_tol=1e-9)
                gap_LP = res_LP.gap()
            except (ValueError, UnboundLocalError): # If linprog fails, just skip this linprog step and use EG instead
                pass
