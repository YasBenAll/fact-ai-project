...
# create a dictionary of the unique values for each column. the key is the number of unique values in the column.
...
# create a list of tuples for the reference data and the data
...
# remove the columns from the reference data that are not in the data
filtered_ref = ref[:,ref_dat_map[:,0]].T 
filtered_dat = { k:v[ref_dat_map[:,1]] for k,v in data_flat.items() } 

# find the indices of the columns in the data that match the reference data
...
# create a list of labels in the same order as the reference data
